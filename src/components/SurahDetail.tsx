...
export function SurahDetail({ nomor, onSelectSurah }: SurahDetailProps) {
  ...
  const playAyah = useCallback(
    (ayahNumber: number, audioUrl: string, isAuto: boolean = false) => {
      ...
      /* Jika sedang auto-play dan user klik ayah manapun → hentikan auto-play */
      if (isAutoPlayingRef.current) {
        stopAutoPlay();
        return;
      }

      /* Jika klik ayah yang sama dan bukan auto-play → jeda/toggle */
      ...
      if (isAuto) {
        const queue: QueueItem[] = (detail?.ayat || [])
          ...
          .map((a) => {
            ...
            if (url) return { ayahNumber: a.nomorAyah, url };
            return null;
          })
          .filter(Boolean);
  ...
  return (
    <>
      {/* Elemen audio tersembunyi (tidak terlihat) */}
      ...
      <div className="surah-detail-header">
        ...
        {(detail.ayat || []).map((ayah) => (
          <AyatCard
            key={ayah.nomorAyat}
            ayah={ayah}
            isPlaying={playingAyah === ayah.nomorAyah}
